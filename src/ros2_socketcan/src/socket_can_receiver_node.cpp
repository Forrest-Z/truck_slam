...
: lc::LifecycleNode("socket_can_receiver_node_imu", options)
{
  interface_ = this->declare_parameter("interface", "can1");
...
LNI::CallbackReturn SocketCanReceiverNode::on_configure(const lc::State & state)
{
  ...
  frames_pub_ = this->create_publisher<can_msgs::msg::Frame>("from_can_bus1", 500);
